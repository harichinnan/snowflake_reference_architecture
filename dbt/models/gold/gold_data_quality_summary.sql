...
latest_dq as (
    ...
    select
        ...
        model_name,
        test_name,
        status                                 as dq_status,   -- PASS | FAIL | WARN
        failed_row_count                       as failed_record_count,
        created_at                             as last_evaluated_at,  -- DQ result ts
...
quarantine_rollup as (
    ...
    select
        ...
        -- QUARANTINE_RECORD has no model_name; the originating object is
        -- source_table. Surface it under model_name for a unified summary grain.
        ...
        max(created_at)                        as last_evaluated_at,  -- record ts
        quarantine_reason,
        count(*)                               as quarantine_count
    from quarantine
    group by source_table, quarantine_status, quarantine_reason
